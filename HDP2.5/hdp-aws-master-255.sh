...
aws cloudformation create-stack --stack-name $1 --template-body file:///tmp/HDP-custom-08s-255.json --parameters file:///tmp/parameter-08s.json --tags file:///tmp/tags.json  --capabilities CAPABILITY_IAM | tee stack-${1}.json
...
echo "ambari_host=${AMBARI_HOST} cluster_name=${1} /tmp/hdp-install-08-255.sh"
ambari_host=${AMBARI_HOST} cluster_name=${1} /tmp/hdp-install-08-255.sh
