...
function hivehook_already_installed() {
    if [ -z "$(find $UNRAVEL_HH_DEST -name 'unravel-hive*-hook.jar' 2>/dev/null)" ]; then
        return 1
    else
        return 0
    fi
...
while true ; do
  # nanny loop
  START_AT=\$(date +%s)
  java -server -Xmx2g -Xms2g -jar /usr/local/\${IDENT}/unravel-emr-sensor.jar $CLUSTER_ID_ARG $CHUNK_ARG --unravel-server \$UNRAVEL_HOST $* > \${IDENT}.out  2>&1
...
function install_spark_conf() {
    ...
    if [ ! -d $conf_dest ]; then
        echo "Spark config directory does not exist. Skipping spark config installation" | tee -a ${OUT_FILE}
        return
    fi
    ...
    local hdfs_url=$(cat $HADOOP_CONF/core-site.xml | grep -A 2 fs.defaultFS | grep '/value' | sed -e 's|^.*[<]value[>]\(.*\)[<]/value[>].*|\1|')
    ...
    if [ -e $conf_dest/spark-defaults.conf ]; then
        ...
        if [ ! -z "$existing_eventLog_entry" ]; then
            ...
            eventlog_without_file_protocol=$(echo ${eventLog_entry_array[1]} | sed "s;^$protocol_hdfs;;" | sed "s;^$protocol_file;;" | sed "s;^$protocol_s3;;" | sed "s;^$protocol_s3n;;")
...
###############################################################################################
# Checks whether the Unravel Spark sensor has already been installed                          #
#                                                                                             #
# Accepts:                                                                                    #
#  - UNRAVEL_SPARK_DEST                                                                       #
###############################################################################################
function spark_already_installed() {
    if [ -z "$(find $AGENT_JARS -name "*spark-*.jar" 2>/dev/null 1>/dev/null)" ]; then
        return 1
    else
        return 0
    fi
...
function spark_postinstall_check() {
  ...
  if [ -d $SPARK_CONF_DEST ]; then
    ...
    [ ! $? ] && echo "ERROR: Missing spark config modifications for executor probe" | tee -a ${OUT_FILE} && ret = 1

    cat ${SPARK_CONF_DEST}/spark-defaults.conf | fgrep "$DRIVER_AGENT_ARGS" 1>/dev/null 2>/dev/null
    [ ! $? ] && echo "ERROR: Missing spark config modifications for driver probe" | tee -a ${OUT_FILE} && ret = 1
  fi
